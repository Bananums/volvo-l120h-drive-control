...
void main_task(void *arg) {
    ...
    while (1) {
        ...
        ReadToolState(shared_state, &tool_payload);
        ReadFunction(shared_state, &function_payload);

        printf("Got drive.steering %f\n", drive_payload.steering);

        if (xQueueSend(queue, &byte, portMAX_DELAY) == pdTRUE) {
            //TODO look into portMAX_DELAY
